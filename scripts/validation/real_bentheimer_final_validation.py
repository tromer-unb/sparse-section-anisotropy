#!/usr/bin/env python3
"""
Final Bentheimer real-rock validation.
...
RAW convention validated previously:
    0 = pore
    1 = solid
...
Outputs
-------
  bentheimer_final_validation.png
  bentheimer_final_validation.svg
  bentheimer_final_validation_report.txt
  ...
  bentheimer_final_reference_cache/
  bentheimer_final_section_cache/
  bentheimer_final_result_cache/
...
Run
---
  python3 real_bentheimer_final_validation.py
...
The script can also reuse compatible 50k seed results from the existing
bentheimer_test4_cache directory for the five subvolumes calibrated there.
"""
...
PNG_PATH = OUTDIR / "bentheimer_final_validation.png"
SVG_PATH = OUTDIR / "bentheimer_final_validation.svg"
REPORT_PATH = OUTDIR / "bentheimer_final_validation_report.txt"
SUBVOL_CSV_PATH = OUTDIR / "bentheimer_final_subvolumes.csv"
SUMMARY_CSV_PATH = OUTDIR / "bentheimer_final_summary.csv"
RANDOM_CSV_PATH = OUTDIR / "bentheimer_final_random.csv"
SYSTEMATIC_CSV_PATH = OUTDIR / "bentheimer_final_systematic.csv"
REFSEED_CSV_PATH = OUTDIR / "bentheimer_final_reference_seeds.csv"

REF_CACHE_DIR = OUTDIR / "bentheimer_final_reference_cache"
SECTION_CACHE_DIR = OUTDIR / "bentheimer_final_section_cache"
RESULT_CACHE_DIR = OUTDIR / "bentheimer_final_result_cache"
TEST4_CACHE_DIR = OUTDIR / "bentheimer_test4_cache"
    ...
    for r in results:
        ...
        for x in r["sparse"]["random"]:
            row = {
                ...
                "phi_all": x["phi_all"],
                "phi_family_rms": x["phi_family_rms"],
                "major_axis_error_deg": x["major_axis_error_deg"],
                "orientation_reliable": x["orientation_reliable"],
                "q00": x["q00"], "q11": x["q11"], "q22": x["q22"],
                "q01": x["q01"], "q02": x["q02"], "q12": x["q12"],
            }
            rows.append(row)
    return rows
...
def write_report(results, agg, settings, elapsed):
    ...
    with open(REPORT_PATH, "w", encoding="utf-8") as f:
        f.write("Bentheimer final real-rock validation\n")
        f.write("=" * 78 + "\n\n")
        ...
        f.write(f"RAW shape: {RAW_SHAPE}, convention 0=pore, 1=solid\n")
...
def build_figure(results, agg, rep_sections):
    ...
    axes_a[1].text(
        0.5, 1.18,
        "Representative Bentheimer sections",
